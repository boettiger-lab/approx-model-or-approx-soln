...
% Ref.~\cite{may77} points out that the system presented above can be reinterpreted as a simple model for a fishery, where quotas are set using a constant effort rule.
% Here, the natural dynamics of $V$ is purely logistic growth, which is hampered by human harvesting according to the function $F$.
% This way, $H$ is the effort allowed for the sake of quotas.
% \footnote{%
%   The non-linearity of the $F$ function is justified in the fact that, for low very populations $V$, the effort necessary to obtain a constant-size catch grows faster than $H\sim 1/V$.
%   Here, for our case, $H\sim 1/V^2$ is used.
% }
