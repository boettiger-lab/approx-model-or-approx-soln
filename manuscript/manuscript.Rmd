...
For instance, Global Circulation Models (GCMs) such as HadCM3 [@pope2000impact; @gordon2000simulation; @collins2001internal] model earth's climate using 1.5M variables, while the comparably vast potential action space is modeled as one of six emissions scenarios from the IPCC [@riahi2017shared].
...
This way, the DRL agent discovers and exploits, as it were, an effective simplification of the control problem.
We provide some intuition as to how this simplification arises from the dynamics of the system, but leave a more rigorous characterization of this phenomenon for future work.
...
\begin{figure}
  ...
  Here, the parameter $D$ was varied between 0.75 and 1.25, and the values of the other parameters are fixed as in eq. \eqref{eq:param vals}. 
  The three components of the fixed points are plotted separately, as a function of $D$.}
...
\begin{align*}
  (X,Y,Z) \approx 
  &(0.839, 0.054, 0.377), \quad 
    (0.048, 0.774, 0.480), \quad
    (0.359, 0.491, 0.509), \\
    &(0.899, 0.000, 0.303), \quad
    (0.000, 0.818, 0.457), \quad
    (0.666, 0.666, 0.000), \\
...
For our use case, the environment will be a computational model of the population dynamics of a fishery, with the environment state at time $t$ being a vector of all the fish populations, $N_t=(X_t,Y_t,Z_t)$.
At each time step, the agent observes the state $S$ and harvests $X$,
\begin{align*}
  (X_t, Y_t, Z_t) \mapsto ((1-M_X)\cdot X_t, Y_t, Z_t),
\end{align*}
where the fish mortality-from-harvest $M_X=M_X(N_t)$ is the agent's action at time $t$. 
This secures a reward of $M_X(N_t)X$.
Afterwards, the environment undergoes a time step under its natural dynamics given by \eqref{eq:3d model stoch}.
...
In our case, these are real-valued functions on a cube $\pi:[0,2]^{\times 3}\to \mathbb{R}_+$.
The space of all such functions is highly singular, spanning a \emph{non-separable Hilbert space}.
Even restricting ourselves to continuous policy functions, we end up with a set of policies which span the infinite dimensional space $L^2([0,2]^{\times3})$.
...
The state space used was a cube $[0,2]^{\times 3}$, while the policies explored mapped states $N_t$ to a single number, $M_X(N_t) = \pi_\theta(N_t)$, the harvest mortality on species $X$ at that timestep.
