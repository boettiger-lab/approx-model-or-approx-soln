...
Global fisheries management has long been a crucible for both the development of theory and best practices in actual management for many reasons. Fisheries are an essential natural resource that provide the primary source of protein for one in every four humans, and have faced widely documented declines due to overfishing [cite, cite]. 
...
Methods developed in the study and management of fisheries are frequently applied to other areas of ecological management, from concepts such as maximum sustainable yield [], optimal control under observation uncertainty [], to adaptive management [], management strategy evaluation [], to the studies of conservation policy and international negotiations [].
...
Despite such rich development, fundamental concepts and approaches widely considered best practices remain rooted in the analysis of analytically tractable one-dimensional models.
...
Specifically, most fisheries policies are framed in terms of a *constant mortality* policy, (sometimes a simple piece-wise linear function, i.e. a single linear segment, is introduced as a precaution a very low stock sizes), where fishing 'mortality' is defined as the ratio of harvest (catch) to stock size.
(This definition sidesteps much complexity -- fisheries regulation is typically implemented in terms of catch quotas -- i.e. limit to metric tons of a given species that can be harvested in a given season. Thus to determine the quota, fisheries scientists must fist estimate the current biomass, and then multiply by this dimensionless ratio to determine the corresponding harvest quota, $H = F x_t$.  Thus the process of measuring the stock size, a 'stock assessment', gives rise to the harvest quota).
Constant mortality policies have a strong basis in theory as well as practical justifications.

The best known such policy is that of "Maximum Sustainable Yield" (MSY), i.e. the largest possible constant mortality that can be sustained indefinitely without depleting the stock.
MSY remains the basis of much fisheries management, including international treaties such as the 1982 UN Law of the Sea [@UNCLOS1982].
MSY in 1D models [@Schaefer1954]
critique and actual practice, ABC and TAC, (but go lighter on acronyms)
...
When the action and state space are discrete and sufficiently small, computational approaches such as dynamic programming may be used to exhaustively search
