...
esc_sim <- escapement |> 
    rename(escapement = action) |> # data had a naming problem
    filter(escapement == best_e) |> 
    pivot_longer(c("X", "Y", "Z"),
        names_to = "species", values_to = "abundance") |>
...
Figure \ref{fig:tbd} exemplifies this point by comparing one times series of the system state obtained using each of these policies.
...
\begin{figure}
	\includegraphics[width=\linewidth, height = 0.2\textheight]{Figures/timeseries.png}
\caption{
\label{fig:one timeseries}
tbd
}
\end{figure}
...
\begin{figure}
    \begin{subfigure}[b]{0.5\textwidth}
        \includegraphics[width=\linewidth]{Figures/ep_length.png}
        \caption{Episode lengths}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.5\textwidth}
