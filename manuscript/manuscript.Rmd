...
\begin{split}
  ...
  f_Z(N_t) &= \left(f(X_t + DY_t\right) - d_Z)Z_t.
\end{split}
\end{align}
...
For this paper, we will consider four environments corresponding to each of the models considered (Table \ref{tab:models}).
...
After this harvest portion of the time step, the environment changes according to its natural dynamics (see eq. \eqref{eq:3d model}).
...
There, we plot the *median episode reward difference* between the two DRL-based methods we considered, and the optimal CEsc strategy.
...
Moreover, non-stationarity exacerbated the advantage of DRL over CEsc and CMort, which can be expected as non-stationary parameters have been observed to introduce bias into single-species reference point estimation [@burgess2017describing].
