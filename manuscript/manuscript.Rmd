...
# rename some columns for figure purposes:
colnames(msy) <- c('t', 'rep', 'action', 'reward', 'X', 'Y', 'Z')
colnames(ppo) <- c('t', 'rep', 'action', 'reward', 'X', 'Y', 'Z') 
colnames(escapement) <- c('t', 'rep', 'esc_level', 'reward', 'X', 'Y', 'Z') 
opt_escapement <- escapement |> filter(esc_level == 0.62)

# ppo dataset uses step rewards rather than cumulative rewards, which is what is useful here.
# so, switch stuff around:
#ppo |> 
#    group_by(rep) |> 
#    mutate(cumulative_reward = cumsum(reward)) |>
#    within(rm("reward", "1")) |>
#    rename(reward = cumulative_reward)

# set up for plots:

# best constant effort based on the data on ../data/msy.csv.gz
best_action <- 0.0465
msy_sim <- msy |>
...
best_e <- 0.62 # optimal constant escapement, computed from ../data/escapement.csv.gz
esc_sim <- escapement |> 
    pivot_longer(c("X", "Y", "Z"),
        names_to = "species", values_to = "abundance") |>
...
\begin{figure}
  \centering
  \includegraphics[width = 0.8\linewidth]{figures/conceptual-figure-old.png}
  \caption{
    \label{fig:conceptual}
    (A) Classical management strategies often have the common aspect of reducing the complex dynamics of the fishery ecosystem to a single equation governing the harvested population (say, $V1$). where the interaction between $V1$ and its environment is summarized to a set of parameters, such as the growth rate $r$, and the carrying capacity $K$. An advantage of one-dimensional approaches is that the optimal policy is often known exactly, and, moreover, is intuitive. For example, in the logistic equation shown, the maximal sustainable yield of the system is attained at K/2. (B) RL allows us to solve for quantitative decision strategies considering more complex ecosystem dynamics. For example, in this paper, we find harvest strategies for a three-species model using a model-free RL algorithm, PPO. (C) We show how the RL-based solution that considers all three species finds harvest policies with higher average reward and with (D) lower probability of harvest species collapse than the classical approaches, only considering the dynamics of V1.
  }
\end{figure}
...
<!--
```{r conceptual, echo = FALSE, message=FALSE, fig.align='center', fig.cap='(A) Classical management strategies often have the common aspect of reducing the complex dynamics of the fishery ecosystem to a single equation governing the harvested population (say, $V1$). where the interaction between $V1$ and its environment is summarized to a set of parameters, such as the growth rate $r$, and the carrying capacity $K$. An advantage of one-dimensional approaches is that the optimal policy is often known exactly, and, moreover, is intuitive. For example, in the logistic equation shown, the maximal sustainable yield of the system is attained at K/2. (B) RL allows us to solve for quantitative decision strategies considering more complex ecosystem dynamics. For example, in this paper, we find harvest strategies for a three-species model using a model-free RL algorithm, PPO. (C) We show how the RL-based solution that considers all three species finds harvest policies with higher average reward and with (D) lower probability of harvest species collapse than the classical approaches, only considering the dynamics of V1.', out.width='1\\linewidth', fig.pos='H'}
knitr::include_graphics("Figures/conceptual-figure-old.png")
```
-->
...
```{r classical_tuning, fig.cap = "TBD.  Red = MSY, Green = 0.8 TAC", out.width = "6.5in"}
msy_rewards <- msy |>
  group_by(action, rep) |>
  filter(t == max(t)) |> 
  group_by(action) |>
...
esc_rewards <- escapement |>
  group_by(esc_level, rep) |>
  filter(t == max(t)) |> 
  group_by(esc_level) |>
  summarise(mean_reward = mean(reward),
            sd = sd(reward)) 
...
rmsy <- msy_rewards |>
  ...
  geom_vline(aes(xintercept=0.0465), color="red") + 
  geom_vline(aes(xintercept=0.0370), color="green") 
...
Each episode in the training and in the evaluation stages was at most 600 time steps long.
...
```{r linear_tuning, fig.cap = "Result of the tuning procedure for the constant mortality strategy. (A) Mean episode reward (averaged over 50 episodes) shown as scatter data, $2\times$ standard deviation around each scatter point shown as a grey shade. Vertical lines correspond to the optimal constant mortality rate $F^* = 0.465$ (red) and $0.8 F^*=0.0372$ (green). (B) Several constant mortality policies shown for illustrative purposes. Slopes $0.02$, $0.03$, $0.055$ and $0.065$ shown in black; the optimal linear policy $F^*X$ shown in red, while $0.8F^*X$ shown in green.", out.width = "6.5in"}
msy_rewards <- msy |>
  group_by(action, rep) |>
  filter(t == max(t)) |> 
  group_by(action) |>
...
rmsy <- msy_rewards |>
  ...
  theme(aspect.ratio = 1/1) + 
  geom_vline(aes(xintercept=0.0465), color="red") + 
  geom_vline(aes(xintercept=0.0372), color="green") +
...
In Fig. \ref{fig:timeseries} we show one example time series for each of the three controls we analyzed: 
the left panel uses an optimalconstant mortality control $F^*X$, the middle panel uses $0.8 F^* X$, while the right panel is controlled by the trained DRL agent.
We see that that optimal constant mortality control leads to a near-extinction event.
...
```{r timeseries, fig.cap="TBD.", out.width = "6.5in"}
policy_labels <- list("msy" = "Opt. Const. Mortality", "tac"= "80% Opt. Const. Mortality", "ppo"= "DRL")
...
```{r rew_len_fig, fig.cap="TBD.", out.width = "6.5in"}
library(patchwork)

p1 <- msy_tac_ppo |> 
    group_by(rep, policy) |>
    filter(t==max(t), rep < 30) |>
    ggplot(aes(rep, t)) + 
      geom_col() +
      facet_wrap(~factor(policy, levels = c("msy", "tac", "ppo")), labeller = policy_labeller ) +
...
p2 <- msy_tac_ppo |> 
    ...
    ggplot(aes(rep, reward)) + 
      geom_col() +
      facet_wrap(~factor(policy, levels = c("msy", "tac", "ppo")), labeller = policy_labeller ) +
      labs(y = "episode reward", x = "repetition") +
      theme(axis.text.x = element_blank())
