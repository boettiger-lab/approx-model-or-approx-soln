...
*Model 1:* here, only $X$ is harvested, that is, in the notation of eq. \eqref{eq:general model}, we fix $M_Y=M_Z=0$ and leave $M_X$ as a control variable.
All parameters here are constant.
*Model 2:* as Model 1, but with $X$ and $Y$ being harvested. 
In other words, we set $M_Z=0$ and leave $M_X$ and $M_Y$ as control variables.
*Model 3:* here $X$ and $Y$ are harvested, but now we include a non-stationary parameter:
\begin{equation}
  \label{eq:rx(t)}
  r_X = r_X(t) = 
  \begin{cases}
...
As a part of that effort, we will attempt capture more detailed models of the effects of climate change.
