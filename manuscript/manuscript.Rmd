...
While simplified models have historically allowed for exploration of large decision spaces, the new wave of rich models are applied to highly oversimplified descriptions of potential actions they seek to inform. <!-- ML: Can we clarify that we are talking about dynamics/process models here? Model is a very overloaded term for me-->
...
This reward function balanced the extractive motivation of the fishery with conservation goals which went beyond the scope of long-term sustainable harvests.
...
```{r}
#| label: ephistograms
#| fig.cap: "Histograms of episode lengths and rewards for the four different management strategies considered. Only the first 50 evaluation episodes (from a total of 100) were included, for ease of visualization."
#| out.width: "6in"
esc_len <- esc_ends %>%
  filter(rep<50) %>%
  ggplot(
...
## Pasted to-be-discussed

Eqs. \eqref{eq:3d model} are a generalization of the single-species model \eqref{eq:may}: this can be seen by setting 
\begin{align*}
  c_{XY} = d = f = \sigma_Z = 0.
\end{align*}
Throughout most of our paper, we use the parameter values given in eq. \eqref{eq:param vals}.
In particular, $c_{XY} = d = f = 0.1$ and $\sigma_Z=0.05$ so, in broad strokes, our three-species model is a slight perturbation of the single-species model.
...
In contrast, DRL optimizes over the more expressive -- and more complicated -- family of policies parameterized by neural networks.
Because of this, our DRL approach uses a variant of *gradient ascent* (the proximal policy optimization algorithm) to obtain a good candidate policy. <!-- ML: I don't think this sentence is the most helpful here. Might just be better to say something like how NN's do well in approximating just about any function empirically-->

We showed that for sufficiently complex management scenarios -- with multiple harvested species and, possibly, time-varying parameters -- DRL-based management strategies perform significantly better than constant escapement.
This is with respect to both average rewards received and conservation goals.
...
What is more, this DRL advantage happened within a regime of parameter space where species interaction terms were much smaller than the single-species logistic growth terms.
Indeed, interaction parameters in \eqref{eq:3d model} are about an order of magnitude smaller than $r_X$ and $r_Y$.
In this situation logistic growth is a reasonable approximation of the system, and thus a constant escapement solution is a plausible control strategy.

We found that the optimal constant mortality policy surprisingly performs much better than constant escapement (Fig. \ref{fig:main}).
...
```{r}
#| label: main_32
#| fig.cap: "Reward distributions for the four strategies considered. These are based on 100 evaluation episodes of the ‘‘three species, two fisheries’’ model in Table 1. We denote CEsc for constant escapement, CMort for constant mortality, PPO for the output policy of the PPO optimization algorithm, and PPO GP for the Gaussian process interpolation of the PPO policy."
#| out.width: "6in"
summary_32 %>% 
  ggplot() +
  geom_violin(mapping = aes(x=reward,y=strategy,fill = strategy)) +
...
```{r}
#| label: main_31
#| fig.cap: "Reward distributions for the four strategies considered. These are based on 100 evaluation episodes of the ‘‘three species, one fishery’’ model in Table 1. We denote CEsc for constant escapement, CMort for constant mortality, PPO for the output policy of the PPO optimization algorithm, and PPO GP for the Gaussian process interpolation of the PPO policy."
#| out.width: "6in"
summary_31 %>% 
  ggplot() +
  geom_violin(mapping = aes(x=reward,y=strategy,fill = strategy)) +
...
```{r}
#| label: main_11
#| fig.cap: "Reward distributions for the four strategies considered. These are based on 100 evaluation episodes of the ‘‘one species, one fishery’’ model in Table 1. We denote CEsc for constant escapement, CMort for constant mortality, PPO for the output policy of the PPO optimization algorithm, and PPO GP for the Gaussian process interpolation of the PPO policy."
#| out.width: "6in"
summary_11 %>% 
  ggplot() +
  geom_violin(mapping = aes(x=reward,y=strategy,fill = strategy)) +
...
## PPO policy function for non-stationary model
