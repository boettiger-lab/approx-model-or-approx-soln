...
\label{eq:3d model}
\begin{split}
  \Delta X_t &= L(X_t;\ r_X, K_X) - F(X_t, Z_t;\ \beta, c) - c_{XY}X_tY_t,\\
...
In this paper we do not set out to provide a realistic model of any given fishery, but rather to show that the DRL approach may be applicable to a broad variety of plausible fish recruitment models.
Because of this, we also provide a *stability analysis*, where we perturb the parameter values randomly.
Finally, in the time-varying model, we use the following drift on the growth rate of $X$:
\begin{equation*}
