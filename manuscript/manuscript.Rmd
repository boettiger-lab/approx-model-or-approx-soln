...
#| out.width: "6in"
frac_msy %>%
  ggplot(aes(x=msy_fraction)) + 
  geom_line(aes(y=reward)) +
  geom_errorbar(aes(ymin=reward-reward_std, ymax=reward+reward_std), width=.01,
                 position=position_dodge(0.05)) +
  geom_line(aes(y=num_full_eps)) +
  geom_point(aes(y=reward)) + 
  geom_point(aes(y=num_full_eps)) + 
  scale_y_continuous(
    
    # Features of the first axis
    name = "% full-length eps",
    
    # Add a second axis and specify its features
    sec.axis = sec_axis(~.+20, name="Mean ep. reward")
  )+
  labs(x="% MSY constant mortality")
```
