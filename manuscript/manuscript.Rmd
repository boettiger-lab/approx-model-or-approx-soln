...
\begin{align}
  ...
  N_{t+1} - N_t = f(N_t) - F N_t := \Delta N_t,
\end{align}
where $N_t = (X_t,\ Y_t,\ \dots) \in \mathbb{R}^{n}_+$ is a vector of populations, $f:\mathbb{R}^{n}\to\mathbb{R}^{n}$ is an arbitrary function, where the product of the mortality and the stock size, $F N_t$ is the harvest size at timestep $t$. 
...
$$
Which combined with the harvest term from \eqref(eq:general model) is known as the Gordon-Schaefer model, after foundational work which first developed the concept of maximum sustainable yield, MSY, [@Gordon1954; @Schaefer1954], which remains a central concept in fisheries management.  
In this model, note that the maximum growth rate occurs at population size of $N = K/2$, which is approached asymptotically from any positive initial state $N_0 \in (0,K)$ under a constant mortality of $F = r/2$, which thus produces a harvest of $rK/4$ at equilibrium -- the maximum yield sustainable indefinitely. 
...
Eq. \eqref{eq:may} is an interesting study case of a *tipping point* (saddle-node bifurcation) (see Fig. \ref{tbd}).
