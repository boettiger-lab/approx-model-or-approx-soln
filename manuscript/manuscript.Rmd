...
The policies explored were functions from states $N_t$ to either a single number $M_X(N_t) = \pi_\theta(N_t)$, or a pair of numbers $(M_X(N_t), M_Y(N_t)) = \pi_\theta(N_t)$ depending on the environment considered.
...
We have chosen this window to be 200 timesteps, where each discrete time-step represents the dynamical evolution of the system over a year.
_Episodes_ begin at timestep zero and are controlled by the policy and the system's natural evolution until either the maximum timespan is reached, or until a ‘‘near-extinction event’’ occurs---that is, until any of the populations go below a given threshold,
\begin{align}
  \label{eq:thresh}
...
The reward function defining our policy optimization problem has two componenets:
The first is economical: the total biomass harvested over an episode.
The second seeks to reflect conservation goals: if a near-extinction event occurs, the episode is ended early and a negative reward of $-100/t$ was awarded.
