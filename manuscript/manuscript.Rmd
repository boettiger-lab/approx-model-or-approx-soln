...
We consider the problem of devising harvest strategies for a series of ecosystem models (Figure \ref{fig:conceptual-v2}):
...
We strengthen this result with a stability investigation: we show that random perturbations in the model's parameter values used do not significantly vary the conclusion that DRL outperforms constant escapement. <!-- ML: I think would be clearest to specify "in the environment's/dynamics model's param..."-->
...
The other three models are all three-species models with similar dynamics---differing by the amount of species that are harvested, and whether the parameters of the model are fixed or time-dependent. <!--ML: Triple parenthetical dash on purpose or an accident? Noticing this has been done before -->
...
**Is this actually true? Should I maybe erase these last sentences?** <!-- ML: cc Carl, but I think these sentences seem reasonable-->
...
All parameters here are constant, with values given by \eqref{eq:param vals}.
...
Finally, *3. Three species, two fisheries, time-variation:* in this model we allow for the harvest of $X$ and $Y$, as in the previous model, but now we include a time dependent parameter:
\begin{equation}
  \label{eq:rx(t)}
  r_X = r_X(t) = 
  \begin{cases}
  ...
  \end{cases}
\end{equation}
All other parameters are fixed by eq. \eqref{eq:param vals}.
...
In future work we intend to test the DRL framework in realistic models of particular fisheries.
As a part of that effort, we will attempt to capture more detailed models of the effects of climate change.
For this exploratory work we have chosen this simple expression, however, for the sake of conceptual clarity.
...
As mentioned previously, this discretized time framework allows a simplification of the possibly seasonal mating behavioral patterns of the species involved. 
...
As seen in Fig. \ref{fig:conceptual-v2}, we consider four different models of the ecosystem with which the fishery interacts, each of these is called an *environment*, alluding to the environment in an RL setting.
We summarize these models in Table \ref{tab:models}.
...
\begin{table}
  \begin{center}
    \begin{tabular}{|l|c|c|c|c|}
      ...
      \hline
      1 species, 1 fishery & \eqref{eq:may} & 1 & $X$ & Yes \\
      3 species, 1 fishery & \eqref{eq:3d model} & 3 & $X$ & Yes \\
      3 species, 2 fishery & \eqref{eq:3d model} & 3 & $X$ and $Y$ & Yes \\
      3 species, 2 fishery, non-stationary & \eqref{eq:3d model} & 3 & $X$ and $Y$ & No \\
      \hline
    \end{tabular}
...
Our goal is to evaluate the performance of different policies over a specified window of time.
...
The second seeks to reflect conservation goals: if a near-extinction event occurs at time $t$, the episode terminates early and a negative reward of $-100/t$ is received.
...
*1)* for the single species model, a line segment $[0,1]$, *2)* for all three-species models, a cube $[0,1]^{\times 3}$.
...
Policies obtained from the PPO algorithm can be ‘‘noisy’’ as their optimization algorithm is randomized (see, e.g. App. B for visualizations of the PPO policy in the ‘‘3 species, 2 fisheries, non-stationary’’ case).
...
The raw dataset is found at the data/results\_data/2FISHERY/RXDRIFT subdirectory in the repository with the source code and data linked above.
...
For each one of these choices of a constant mortality rate, say $M_X$, we simulated 100 episodes based on \eqref{eq:3d model}:
At each time step the state $(X_t, Y_t, Z_t)$ was observed, a harvest of $M_X X_t$ was collected and then the system evolved according to its natural dynamics \eqref{eq:3d model}.
...
The three-species model's (eq. \eqref{eq:3d model}) dynamic parameters were chosen as follows
\begin{align}
\label{eq:param vals}
\begin{split}
...
We evaluated each of the four control approaches considered -- MSY constant mortality, optimal constant escapement, PPO, and Gaussian process interpolation of the latter -- on each of the four scenarios examined (see Sec. 3).
...
We will visualize the results concerning the last scenario considered (‘‘3 species, 2 of which harvested, and a time-varying parameter’’) in this section, leaving the other cases for App. A.
...
This conceptually shows how one-dimensional intuitions on ecological control may lose validity for complex ecosystems. **(this might be unwelcome by the reviewers if not discussed properly)**
...
esc_len <- esc_ends %>%
  ...
  geom_col() +
  labs(x="repetition", y="ep. duration")
msy_len <- msy_ends %>%
  filter(rep<50) %>%
  ggplot(
    mapping = aes(x = rep, y = t)
...
We examine the trade-off between profit and sustainability faced by the constant mortality strategy in Fig. \ref{fig:fracmsy}.
We expect a similar and, possibly, more pronounced effect for the constant escapement strategy but do not analyze this case here.
Here, two quantities are plotted: the fraction of evaluation episodes of maximal length (i.e. episodes with no near-extinction events) and the average reward obtained.
...
\end{align*}
where $(M_X^*,\ M_Y^*)$ is the optimal constant mortality strategy.
