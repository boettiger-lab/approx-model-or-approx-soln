...
For our use case, the environment will be a computational model of the population dynamics of a fishery, with the environment state at time $t$ being a vector of all the fish populations, $S_t=(X_t,Y_t,Z_t)$.
At each time step, the agent observes the state $S$ and harvests $X$,
\begin{align*}
  (X_t, Y_t, Z_t) \mapsto ((1-M_X)\cdot X_t, Y_t, Z_t),
\end{align*}
where the fish mortality-from-harvest $M_X=M_X(S_t)$ is the agent's action at time $t$. 
This secures a reward of $M_X(S_t)X$.
