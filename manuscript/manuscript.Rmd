...
*Model 1:* DRL-based strategies recover the optimal CEsc policy function.  
Constant mortality performs considerably worse than these three.\footnote{
As will be explained later, all our models are stochastic. 
If we set stochasticity to zero in Model 1, constant mortality matches the performance of the other management strategies.
}
*Model 2:* Here, all management strategies perform similarly.
*Model 3:* For this model, DRL outperforms both classical strategies, with CMort surprisingly performing significantly better than constant escapement.
...
```{r}
#| label: fracmsy
#| fig.cap: "Trade-off between reward and probability of a near-extinction event for constant mortality policies. We evaluated policies at the full optimal constant escapement value, and also at 0.8, 0.9, 0.95 of the latter. Each evaluation is based on 100 episodes. On the left we plot the percentage of episodes which last their maximum time window, i.e. that do not see a near-extinction event. On the right, we plot the mean episdoe reward and standard deviation for each policy."
#| out.width: "6in"
lengths <- frac_msy %>%
...
While the optimal control for our single-species model \eqref{eq:may} can not be easily proven to be constant escapement (since the right-hand side of that equation is not concave), from experience we can expect constant escapement to either be optimal or near-optimal.
...
We believe the most important aspect of the constant escapement policy reproduced by PPO is the fact that there is some sufficiently-wide window below the threshold of the policy (i.e. below the optimal escapement value), on which no fishing is performed.
...
This provides easily interpretable solutions, such as constant mortality policies.
...
We compare deep reinforcement learning-based policies against classical management strategies (optimal constant mortality and optimal constant escapement policies policies). 
While the latter are inspired by the shape of optimal solutions in the single-species setting, they are optimized in a model-free way as well: e.g. the optimal constant mortality rate is computed empirically from simulated data.
...
We found that the optimal CMort policy surprisingly performs much better than constant escapement (Fig. \ref{fig:main}).
However, it can be observed in Fig. \ref{fig:fracmsy} that the constant mortality strategy faces a trade-off: high sustainability is achieved for sub-optimal constant mortality rates, but only at a significant decrease in the mean episode reward.
...
The question of when multi-species models are well-approximated by single-species models was studied in detail in [@burgess].
...
We do not investigate further whether our three-species models are well approximated by a single-species model in the sense of [@burgess].
However:
...
Moreover, non-stationarity exacerbated the advantage of DRL over CEsc and CMort, which can be expected as non-stationary parameters have been observed to introduce bias into single-species reference point estimation [@burgess].

Finally, we performed a stability analysis to ensure that the advantage of DRL-based techniques over constant escapement is a ubiquitous phenomenon and not a result of a lucky selection of parameter values.
