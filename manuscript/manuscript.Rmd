...
$$
  f(N_t) = r N_t\big(1 - N_t / K \big) =: L(N_t;\ r, K).
$$
More relevant to our current work, is the following single-species model studied in \cite{may77},
\begin{align}
  \label{eq:may}
  N_{t+1} - N_t
  = L(N_t) - F(N_t, H; \beta,c),
\end{align}
where,
\begin{align*}
  F(N,H;\ \beta,c) := \frac{\beta H N^2}{c^2 + N^2}.
\end{align*}
...
\begin{align}
\begin{split}
  \Delta X_t &= L(X_t;\ r_X, K_X) - F(X_t, H_t;\ \beta, c) - c_{XY}X_tY_t - h_t,\\
  \Delta Y_t &= L(Y_t;\ r_X, K_X) - D F(Y_t, H_t;\ \beta, c) - c_{XY}X_tY_t,\\
  \Delta Z_t &= (fX_t + fDY_t - d_Z)Z_t
\end{split}
\end{align}
