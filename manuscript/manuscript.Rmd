...
Here, two quantities are plotted: the fraction of evaluation episodes with maximal length (i.e. episodes with no near-extinction events), and the average reward.
...
lengths <- frac_msy %>%
  ...
  labs(x="% MSY constant mortality", y="% full-length eps.") + 
...
rewards <- frac_msy %>%
  ...
  labs(x="% MSY constant mortality", y="Mean ep. reward")
...
Moreover, in the case of our ‘‘PPO+GP’’ management strategy, we sample $\pi_\theta$ on a grid of points $G=\{(X_i, Y_i, Z_i)\}_i$ and use a Gaussian process to interpolate $\pi_\theta$, i.e.
\begin{align}
  \pi_{\text{PPO+GP}} = \text{Gaussian process interpolation of } \pi_\theta|_G.
\end{align}
This process was explained in detail in the Methods section.

Here we visualize the resulting $\pi_{\text{PPO+GP}}$ policy and provide an interpretation for it.\footnote{
A similar characterization of $\pi_\theta$ is found on the appendix. 
Here we focus on the Gaussian process interpolation as it is smoother and, thus, easier to interpret.
}
