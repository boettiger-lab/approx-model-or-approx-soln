...
\begin{align}
  ...
  \begin{split}
    \Delta X_t &= f_X(N_t) - M_X X_t\\
    \Delta Y_t &= f_Y(N_t) - M_Y Y_t\\
    &\dots
  \end{split}
\end{align}
where $N_t = (X_t,\ Y_t,\ \dots) \in \mathbb{R}^{n}_+$ is a vector of populations, $\Delta X_t := X_{t+1} - X_t$, $\Delta Y_t := Y_{t+1} - Y_t$, and where $f_i:\mathbb{R}^{n}\to\mathbb{R}$ are arbitrary functions.
...
The model has five parameters: the growth rate $r$ and carrying capacity $K$ for $X$, a constant population $H$ of a species which preys on $X$, the maximal predation rate $\beta$, and the predation rate half-maximum biomass $c$.
...
Consider the following three species recruitment model which generalizes eq. \eqref{eq:may}:
\begin{align}
\label{eq:3d model}
\begin{split}
  \Delta X_t &= L(X_t;\ r_X, K_X) - F(X_t, Z_t;\ \beta, c) - c_{XY}X_tY_t - h_{X,t},\\
  \Delta Y_t &= L(Y_t;\ r_Y, K_Y) - D F(Y_t, Z_t;\ \beta, c) - c_{XY}X_tY_t - h_{Y,t},\\
...
Finally, a death rate $d_Z$ and a parameter $f$ scaling the birth rate of $Z$.

As mentioned in the introduction, we consider two harvesting scenarios: one where only $X$ is harvested (i.e. $h_{Y,t}=0$), and one where both $X$ and $Y$ are harvested.
For most of our paper, parameters used in this model will be given as in eq. \eqref{eq:param vals}.
In the time-varying model, however, we use the following drift on the growth rate of $X$:
\begin{equation*}
  r_X = r_X(t) = \max(1-t/200, 1/2).
\end{equation*}
...
\begin{align*}
  \Delta X_t &= L(X_t;\ r_X, K_X) - F(X_t, H;\ \beta, c),\\
...
Here, a fixed point analysis provides a less intuitive picture of the dynamics---it no longer necessarily alternates in a tipping manner between two stable points.
...
This way, instead of eq. \eqref{eq:3d model}, we consider the problem of harvesting the following stochastic model:
<!--
\begin{align}
  \label{eq:may stoch}
  \Delta X_t
...
\end{split}
\end{align}
Here, $\{\eta_{i,t}\}$ is a set of independent Gaussian variables centered at zero with variances, respectively, $\sigma_X^2$, $\sigma_Y^2$ and $\sigma_Z^2$.
...
After this harvest portion of the time step, the environment changes according to its natural dynamics given by \eqref{eq:3d model stoch}.
...
To recap, these four environments are a single-species growth model \eqref{eq:may77} for a harvested species; a three-species model \eqref{eq:3d} with a single harvested species; the same three-species model but with *two* harvested species; and, finally, a three-species model with a time-varying parameter and two harvested species.

The policies explored were functions from states $N_t$ to a single number, $M_X(N_t) = \pi_\theta(N_t)$, in the single fishery case.
In the two-fishery case, policies lead to two-dimensional actions: $(M_X(N_t), M_Y(N_t)) = \pi_\theta(N_t)$.
...
\begin{align}
  ...
  Z \leq Z_{\text{thresh.}}.
\end{align}
For a realistic fishery model, this threshold value could represent a population value beyond which drastic ecological consequences may ensue, e.g. a tipping point.
In our setting we have chosen $X_{\text{thresh.}}=Y_{\text{thresh.}}=Z_{\text{thresh.}}=0.05$ as a rule of thumb---given that under natural (unharvested) dynamics the populations range within values of 0.5 to 1, this would represent on the order of a 90% population decrease from its ‘‘natural’’ level.

The reward function had two componenets:
First, at each time step the harvest $h_t$ was received as a reward.
Second, if the episode ended early (say, at time-step $t$) because of a near-extinction event, a negative reward of $-100/t$ was awarded.
...
We trained a DRL agent parametrized by a neural network $\theta$ with two hidden, 64-neuron, layers, on a local server with 2 commercial GPUs.
...
A grid of 100 mortality rates was laid in the interval $[0, 0.5]$.
...
$$
where the higher value of $X_{\text{thresh.}}$ helped select for more sustainable fishing strategies.
