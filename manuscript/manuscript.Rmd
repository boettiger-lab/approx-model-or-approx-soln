...
This is the case since these policy functions are only specified by one or two parameters (in the single fishery, or two fishery cases).
...
We showed that for sufficiently complex management scenarios---with multiple harvested species and, possibly, time-varying parameters---, optimal constant escapement can perform poorly both in terms of reward (which may be used as a proxy for economic performance), as in terms of sustainability (i.e. probability of a near-extinction).
In the case we investigated in detail, the optimal constant mortality policy surprisingly performs much better than constant escapement.
We nonetheless observe that the constant mortality strategy is faced with a trade-off: higher sustainability is achieved for sub-optimal constant mortality rates, but only at a significant decrease in the mean episode reward.
We expect that with increasing complexity this phenomenon might become more pronounced.
This is because of the rigidity of classical strategies: the simplicity of their expressions, depending only on a few parameters, means that policy optimization is constrained to a rather reduced subset of the space of possible policies.

For the most complicated ecosystem models we considered (those with three species, two of which are harvested) we saw a significant improvement in both average reward and sustainability when using DRL-based strategies.
In particular, there was a stark difference in performance between both DRL-based strategies tested, and constant escapement---the optimal solution for a wide array of single-species models.
This, in addition, within a regime of parameter space where species interaction terms about an order of magnitude smaller than the single-species logistic growth---i.e., in a regime where logistic growth could be considered a reasonable approximation of the system.
