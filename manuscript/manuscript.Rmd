...
In the time-varying model, moreover, we use the following drift on the growth rate of $X$:
\begin{equation}
  \label{eq:rx(t)}
...
To recap, these four environments are a single-species growth model \eqref{eq:may77} for a harvested species; a three-species model \eqref{eq:3d model stoch} with a single harvested species; the same three-species model but with *two* harvested species; and, finally, a three-species model with a time-varying parameter and two harvested species.
...
The single-species model's (eq. \eqref{eq:may77}) dynamic parameters were chosen as
\begin{align}
\label{eq:1d param vals}
r = K = 1, \beta = 0.25, c = 0.1.
\end{align}
...
Measure the average reward difference between strategies for each of these parameter choice samples.
Average this difference over the 100 samples.
