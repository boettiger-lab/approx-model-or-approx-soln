...
#| On the x-axis are four different fishery management problems: a single species model, and three three-species models.
#| From left to right, the latter are: three species out of which one, X, is harvested, and all parameters are constant in time; 
#| three species out of which two, X and Y, are harvested and all parameters are constant in time;
#| and finally three species out of which two, X and Y, are harvested and one parameter depends on time. 
#| We represent the latter’s time-dependence with a clock next to the X variable, and we intend to use it as an example of a possible simplified model for the effects of climate change.
#| On the y-axis we have different management strategies with which one may control each of the models. 
#| On the bottom we have the constant mortality strategy (CMort), where one optimizes over constant fishing effort strategies.
#| Above that is the constant escapement strategy (CEsc), based on calling off all fishing below a certain threshold population value.
...
knitr::include_graphics("figures/conceptual-v2.jpeg")
```
...
*Model 4:* Here, the performance gap between DRL and classical management strategies is broadened.
In this case, as in Model 3, constant CEsc performs significantly worse than DRL and CMort.
