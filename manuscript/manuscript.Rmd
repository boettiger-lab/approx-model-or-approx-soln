...
The state space used was a cube $[0,1]^{\times 3}$, while the policies explored mapped states $N_t$ to a single number, $M_X(N_t) = \pi_\theta(N_t)$, the harvest mortality on species $X$ at that timestep.
...
The constant 100 was chosen so as to select against highly extractive policies in the early stages of training, and thus guide the agent towards sustainable policies.
...
Thus, we normalized state space (to be contained in a cube of length 1) and engaged in reward shaping in order to optimize our agent's learning.
